...
namespace App\Library\Admin\Backup;
...
class MysqlBackup
{
    ...
    public function backup($table, $start){
        ...
        if(0 == $start){
            ...
            $result = get_object_vars(reset($result));
            $sql  = "\n";
            $sql .= "-- -----------------------------\n";
        ...
        if($count){
            ...
            foreach ($result as $row) {
                $row = array_map('addslashes', get_object_vars($row));
    ...
    public static function optimize($tables)
    {
        ...
        return DB::statement("OPTIMIZE TABLE `{$tables}`");
    }
    ...
    {
        $tables = is_array($tables) ? implode('`,`', $tables) : $tables;
        return DB::statement("REPAIR TABLE `{$tables}`");
    }
